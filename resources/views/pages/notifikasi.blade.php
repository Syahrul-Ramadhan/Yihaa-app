...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <form action="{{ route('notifikasi.markAllAsRead') }}" method="POST">
            @csrf
            <button type="submit"
                class="flex items-center gap-2 px-4 py-2 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white text-sm font-semibold rounded-lg transition">
    ...
    <!-- Notifications List -->
    @if(empty($notifications))
    <div class="flex flex-col items-center justify-center ml-50 mt-30 space-y-2">
        ...
        <div x-data="{ expanded: false }"
            ...
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex gap-3 flex-1" @click="expanded = !expanded" style="cursor: pointer;">
                    <!-- Avatar -->
                    <img src="{{ $notif['users']['avatar_url'] ?? 'https://ui-avatars.com/api/?name=User' }}" alt="User"
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="font-medium {{ $notif['is_read'] ? 'text-gray-400' : 'text-white' }}">
                            ...
                            <span class="font-semibold text-[#2aa3ef]">{{ $notif['users']['name'] ?? 'Someone' }}</span>
                            liked your post
                        ...
                        <div x-show="expanded" x-collapse class="mt-3 pt-3 border-t border-gray-700">
                            ...
                            <div class="flex gap-2 mt-3">
                                <form
                                    action="{{ route('notifikasi.acceptTeam', [$notif['notification_id'], $notif['team_id']]) }}"
                                    method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg transition">
                                        <i class="hgi hgi-stroke hgi-checkmark-circle-02"></i>
                                        Accept
                                    </button>
                                </form>
                                <form
                                    action="{{ route('notifikasi.rejectTeam', [$notif['notification_id'], $notif['team_id']]) }}"
                                    method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-lg transition">
                ...
                <div class="flex gap-2 flex-shrink-0">
                    ...
                    <form action="{{ route('notifikasi.markAsRead', $notif['notification_id']) }}" method="POST">
                        @csrf
                        <button type="submit" class="p-2 hover:bg-[#2aa3ef20] rounded-lg transition"
                            title="Mark as read">
...
<div id="alertModal" class="fixed inset-0 bg-[#00000031] bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div

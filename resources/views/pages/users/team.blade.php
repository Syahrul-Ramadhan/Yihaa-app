...
<div>
    ...
    <form action="{{ route('teams.index') }}" method="GET" class="relative">
        ...
        <a href="{{ route('teams.index') }}"
            class="absolute inset-y-0 end-0 flex items-center pe-4 text-gray-400 hover:text-white transition">
    ...
    <section class="py-5">
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <a href="{{ route('teams.show', $team['team_id']) }}"
                    class="flex flex-col justify-between bg-gradient-to-br from-[#2aa3ef20] to-[#ffffff0a] backdrop-blur-md border border-[#2aa3ef] p-5 rounded-2xl shadow-md text-white hover:scale-[1.02] hover:shadow-[#2aa3ef40] transition duration-300 cursor-pointer">
                    <div class="flex flex-col gap-2">
                        <!-- Header -->
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="bg-[#2aa3ef40] p-3 rounded-full w-14 h-14 flex items-center justify-center overflow-hidden">
                            ...
                            <span
                                class="px-2 py-1 bg-yellow-500/20 text-yellow-400 text-xs rounded-full border border-yellow-500/40">Leader</span>
                            @endif
                        </div>
                    ...
                    <!-- View Details Button -->
                    <div
                        class="w-full py-2 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white font-semibold rounded-xl transition cursor-pointer text-center">
                        View Details
                    </div>
                </a>
                @endforeach
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <a href="{{ route('teams.show', $team['team_id']) }}"
                class="flex flex-col justify-between bg-[#ffffff0a] backdrop-blur-md border border-[#2aa3ef20] p-5 rounded-2xl shadow-md text-white hover:scale-[1.02] hover:border-[#2aa3ef80] transition duration-300 cursor-pointer">
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex items-center gap-3">
                            <div
                                class="bg-[#2aa3ef20] p-3 rounded-full w-14 h-14 flex items-center justify-center overflow-hidden">
                                @if(isset($team['team_logo']) && $team['team_logo'])
                                <img src="{{ $team['team_logo'] }}" alt="team logo"
                                    class="w-full h-full object-cover rounded-full">
                                @else
...
<div id="createTeamModal"
    class=" hidden fixed inset-0 bg-[#00000059] bg-opacity-70 backdrop-blur-sm z-50 items-center justify-center p-4">
    <div
        class="bg-gradient-to-br from-[#0D1517] to-[#0e1525] rounded-2xl w-full max-w-lg border border-[#2aa3ef40] shadow-2xl">
        <!-- Modal Header -->
        <div class="flex justify-between items-center px-6 py-4 border-b border-[#2aa3ef20]">
            ...
            <button
                onclick="document.getElementById('createTeamModal').classList.add('hidden'); document.getElementById('createTeamModal').classList.remove('flex');"
        ...
        <form action="{{ route('teams.store') }}" method="POST" enctype="multipart/form-data"
            class="max-h-100 overflow-y-auto p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <div id="logoPreview"
                        class="bg-[#2aa3ef20] rounded-xl w-24 h-24 flex items-center justify-center overflow-hidden border-2 border-[#2aa3ef40]">
                        <i class="hgi hgi-stroke hgi-group text-4xl text-[#2aa3ef]"></i>
                    </div>
                    <label for="team_logo"
                        class="flex-1 px-5 py-3 bg-[#2aa3ef20] hover:bg-[#2aa3ef30] border border-[#2aa3ef] text-[#2aa3ef] rounded-lg cursor-pointer transition text-center font-medium">
                        Choose Image
            ...
            <!-- Submit Button -->
            <button type="submit"
                class="w-full py-3 bg-gradient-to-r from-[#2aa3ef] to-[#1e8ac9] hover:from-[#1e8ac9] hover:to-[#2aa3ef] text-white font-bold rounded-lg transition shadow-lg shadow-[#2aa3ef30] hover:shadow-[#2aa3ef50]">
                Create Team
            </button>
        </form>
    </div>
    ...
    document.getElementById('team_logo')?.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                preview.innerHTML =
                    `<img src="${e.target.result}" alt="Logo Preview" class="w-full h-full object-cover rounded-xl">`;
            };

...
<div class="max-w-4xl mx-auto">
    ...
    <a href="{{ route('teams.index') }}"
        class="inline-flex items-center gap-2 text-white mb-6 hover:text-[#2aa3ef] transition">
            ...
            <div class="flex-1">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('chat.show', $team['team_id']) }}"
                        class="flex items-center gap-2 px-6 py-3 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white font-semibold rounded-xl transition cursor-pointer">
                        <i class="hgi hgi-stroke hgi-message-02"></i>
                        Open Chat
                    </a>

                    @if($team['leader_id'] == session('user_id'))
                    <button onclick="showEditTeamModal()"
                        class="flex items-center gap-2 px-6 py-3 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white font-semibold rounded-xl transition cursor-pointer">
                        <i class="hgi hgi-stroke hgi-pencil-edit-02"></i>
                        Edit Team
                    </button>
                    <button onclick="document.getElementById('deleteTeamModal').classList.remove('hidden')"
                        class="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition cursor-pointer">
                    ...
                    @elseif($isPending)
                    <button disabled
                        class="flex items-center gap-2 px-6 py-3 bg-gray-600 text-gray-300 font-semibold rounded-xl cursor-not-allowed">
                        <i class="hgi hgi-stroke hgi-time-02"></i>
                        Request Pending
                    </button>
                    @else
                    @if($team['member_count'] >= $team['member_limit'])
                    <button onclick="showTeamFullModal()"
                        class="flex items-center gap-2 px-6 py-3 bg-gray-600 text-gray-300 font-semibold rounded-xl">
                        <i class="hgi hgi-stroke hgi-lock-02"></i>
                        Team Full
                    </button>
                    @else
                    <form action="{{ route('teams.join', $team['team_id']) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="px-6 py-3 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white font-semibold rounded-xl transition cursor-pointer">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-3 bg-[#ffffff05] rounded-lg">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button onclick="showKickModal('{{ $member['user_id'] }}', '{{ $member['name'] }}')"
            ...
            <div class="flex items-center justify-between p-3 bg-[#ffffff05] rounded-lg">
                <div class="flex items-center gap-3">
                    <img src="{{ $pending['avatar_url'] ?? 'https://qdfotopajdiuailyeprh.supabase.co/storage/v1/object/public/avatars/default-user.jpg' }}"
                        alt="{{ $pending['name'] }}" class="w-10 h-10 rounded-full object-cover">
                    <div>
                ...
                <div class="flex gap-2">
                    <form action="{{ route('teams.acceptMember', [$team['team_id'], $pending['user_id']]) }}"
                        method="POST">
                        @csrf
                        <button type="submit"
                            class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg transition">
                            <i class="hgi hgi-stroke hgi-checkmark-circle-02 mr-1"></i>
                            Accept
                        </button>
                    </form>
                    <form action="{{ route('teams.rejectMember', [$team['team_id'], $pending['user_id']]) }}"
                        method="POST">
                        @csrf
                        <button type="submit"
...
<div id="alertModal" class="fixed inset-0 bg-[#0000003d] bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div
        class="bg-[#0D1517] rounded-2xl p-6 w-full max-w-md border {{ session('success') ? 'border-green-500/30' : 'border-red-500/30' }} transform scale-100 animate-[slideUp_0.3s_ease-out]">
        <div class="flex items-center gap-3 mb-4">
            <div
                class="w-12 h-12 {{ session('success') ? 'bg-green-500/20' : 'bg-red-500/20' }} rounded-full flex items-center justify-center">
                <i
                    class="hgi hgi-stroke {{ session('success') ? 'hgi-checkmark-circle-02 text-green-500' : 'hgi-alert-02 text-red-500' }} text-2xl"></i>
            </div>
        ...
        <button onclick="document.getElementById('alertModal').remove(); window.location.reload();"
            class="w-full px-4 py-3 {{ session('success') ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700' }} text-white font-semibold rounded-lg transition cursor-pointer">
            OK
        </button>
    </div>
</div>
...
<!-- Delete Team Modal -->
<div id="deleteTeamModal" class="hidden fixed inset-0 bg-[#00000038] bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-[#0D1517] rounded-2xl p-6 w-full max-w-md border border-red-500/30">
            ...
            <p class="text-gray-300 mb-6">
                Are you sure you want to delete this team? This action cannot be undone and will remove all team
                members.
            </p>

            <div class="flex gap-3">
                <button onclick="document.getElementById('deleteTeamModal').classList.add('hidden')"
                    class="flex-1 px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg transition cursor-pointer">
                    Cancel
                </button>
                <form action="{{ route('teams.destroy', $team['team_id']) }}" method="POST" class="flex-1">
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition cursor-pointer">
                        Delete
                    </button>
                </form>
            </div>
...
<div id="kickMemberModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div
            class="bg-[#0D1517] rounded-2xl p-6 w-full max-w-md border border-red-500/30 transform scale-100 animate-[slideUp_0.3s_ease-out]">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-12 h-12 bg-red-500/20 rounded-full flex items-center justify-center">
            ...
            <p class="text-gray-300 mb-6">
                Are you sure you want to kick <span id="kickMemberName" class="font-semibold text-white"></span> from
                the team?
            </p>
...
<div id="teamFullModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div
            class="bg-[#0D1517] rounded-2xl p-6 w-full max-w-md border border-yellow-500/30 transform scale-100 animate-[slideUp_0.3s_ease-out]">
            <div class="flex items-center gap-3 mb-4">
            ...
            <p class="text-gray-300 mb-6">
                This team has reached its maximum member capacity. You cannot send a join request at this time. Please
                try again when a spot becomes available.
            </p>

            <button onclick="closeTeamFullModal()"
...
<!-- Edit Team Modal -->
<div id="editTeamModal" class="hidden fixed inset-0 bg-[#0000003f] bg-opacity-50 z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div
            class="bg-[#0D1517] h-120 overflow-y-auto rounded-2xl p-6 w-full max-w-2xl border border-[#2aa3ef30] transform scale-100 animate-[slideUp_0.3s_ease-out]">
            ...
            <form action="{{ route('teams.update', $team['team_id']) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="member_limit" value="{{ $team['member_limit'] }}"
                        ...
                        class="w-full px-4 py-3 bg-[#1a1f23] border border-gray-700 text-white rounded-lg focus:border-[#2aa3ef] focus:outline-none">
                    <p class="text-xs text-gray-500 mt-1">Current members: {{ $team['member_count'] }}. Cannot set limit
                        below current member count.</p>
                </div>

                <div class="flex gap-3">
                    <button type="button" onclick="closeEditTeamModal()"
                        class="flex-1 px-4 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg transition cursor-pointer">
                        Cancel
                    </button>
                    <button type="submit"
                        class="flex-1 px-4 py-3 bg-[#2aa3ef] hover:bg-[#2aa3efcc] text-white font-semibold rounded-lg transition cursor-pointer">
                        Save Changes
                    </button>
                </div>
            </form>

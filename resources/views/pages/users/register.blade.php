...
<div class="flex items-center md:h-auto min-h-screen overflow-hidden">
    <div
        ...
        <div class="flex items-center justify-center ">
            <form id="yihaa-register-form" action="{{ route('register.form') }}" method="POST"
                class="space-y-6 mt-3 w-full max-w-sm">
                @csrf
                <div class="max-w-sm space-y-6">
                    <div class="relative">
                        <input type="text" id="register-name" name="name"
                            class="peer p-4 block w-full border border-gray-300 rounded-lg sm:text-sm placeholder:text-transparent focus:border-[#27D5E8] focus:ring-[#27D5E8] focus:pt-6 focus:pb-2 not-placeholder-shown:pt-6 not-placeholder-shown:pb-2 autofill:pt-6 autofill:pb-2"
                ...
                <button type="submit" id="register-btn"
                    class="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-[#0D1517] text-white hover:bg-[#182427] focus:outline-hidden cursor-pointer disabled:opacity-50">
                    Register
                </button>
            </form>
        </div>
        ...
        <div class="mt-6 text-center">
            <p class="text-base text-gray-800">Already have an account? <a href="/"
                    class="font-bold text-base text-[#27D5E8] underline hover:text-[#198b97]">Login</a></p>
        </div>
    </div>
</div>

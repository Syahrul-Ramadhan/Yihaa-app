...
<div class="flex h-screen bg-[#0E1419] text-white">
    ...
    <div x-data="{ showRef: false }" class="w-80 border-r border-gray-700 flex flex-col">
        ...
        <div class="flex items-center gap-3 p-4 border-b border-gray-700">
            ...
            <button class="text-gray-400 hover:text-white cursor-pointer" @click="showRef = !showRef">
                <i class="hgi hgi-stroke hgi-add-01"></i>
            </button>
        </div>

        {{-- Join Team --}}
        <div x-show="showRef" x-transition class="p-4 border-b border-gray-700">
            <p class="text-sm text-gray-300 mb-2">Join team with referral code</p>
            <div class="flex">
                <input type="text" placeholder="Enter code..." 
                    class="flex-1 bg-[#1A232B] rounded-l-md px-3 py-2 text-sm focus:outline-none">
                <button class="bg-blue-600 px-4 py-2 rounded-r-md text-sm hover:bg-blue-700">Join</button>
            </div>
        </div>
        ...
        @else
            ...
            <div class="p-4 border-t border-gray-700">
                <form action="{{ route('chat.send', $activeTeam['team_id']) }}" method="POST" class="flex gap-3">
                    ...
                    <button type="submit" class="bg-[#2aa3ef] px-6 py-2 rounded-lg hover:bg-[#2aa3efcc] text-white font-semibold transition flex items-center justify-center">
